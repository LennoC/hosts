...
###此脚本配置只需运行一次###
###只适用老毛子华硕固件###
###一键运行命令（初次运行）： mkdir -p /etc/storage/dnsmasq/dns;wget --no-check-certificate https://raw.githubusercontent.com/LennoC/hosts/master/dnsmasq/sh/setting.sh -O /etc/storage/dnsmasq/dns/setting.sh;sh /etc/storage/dnsmasq/dns/setting.sh
echo "————————————开始脚本—————————————"
echo " "
echo "·········写入配置信息···········"
...
wget --no-check-certificate https://raw.githubusercontent.com/LennoC/hosts/master/dnsmasq/sh/start.sh -O /etc/storage/dnsmasq/dns/start.sh
wget --no-check-certificate https://raw.githubusercontent.com/LennoC/hosts/master/dnsmasq/sh/del.sh -O /etc/storage/dnsmasq/dns/del.sh
echo " "
